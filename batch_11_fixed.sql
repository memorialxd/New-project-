...
DROP POLICY IF EXISTS "Drivers view own tx" ON public.transactions;
CREATE POLICY "Drivers view own tx"
  ON public.transactions FOR SELECT
  USING (wallet_kind = 'driver' AND wallet_owner_id = auth.uid());

DROP POLICY IF EXISTS "Customers view own tx" ON public.transactions;
CREATE POLICY "Customers view own tx"
  ON public.transactions FOR SELECT
  USING (wallet_kind = 'customer' AND wallet_owner_id = auth.uid());

DROP POLICY IF EXISTS "Store owners view own store tx" ON public.transactions;
...
DROP POLICY IF EXISTS "Admins manage surge events" ON surge_events;
CREATE POLICY "Admins manage surge events" ON public.surge_events FOR ALL
  USING (has_role(auth.uid(),'admin'::app_role)) WITH CHECK (has_role(auth.uid(),'admin'::app_role));
DROP POLICY IF EXISTS "Drivers view active surge" ON surge_events;
...
DROP POLICY IF EXISTS "Admins manage surge overrides" ON surge_overrides;
...
DROP POLICY IF EXISTS "Admins manage basket rules" ON basket_distribution_rules;
...
DROP POLICY IF EXISTS "Admins view basket distributions" ON basket_distributions;
CREATE POLICY "Admins view basket distributions" ON public.basket_distributions
  FOR SELECT USING (has_role(auth.uid(),'admin'::app_role));
DROP POLICY IF EXISTS "Admins insert basket distributions" ON basket_distributions;
...
DROP POLICY IF EXISTS "Admins view basket payouts" ON basket_distribution_payouts;
CREATE POLICY "Admins view basket payouts" ON public.basket_distribution_payouts
  FOR SELECT USING (has_role(auth.uid(),'admin'::app_role));
DROP POLICY IF EXISTS "Drivers view own basket payouts" ON basket_distribution_payouts;
...
DROP POLICY IF EXISTS "Admins view pricing overrides" ON public.store_pricing_overrides;
...
DROP POLICY IF EXISTS "Host, participants and staff view group orders" ON public.group_orders;
...
DROP POLICY IF EXISTS "Group members and staff view participants" ON public.group_order_participants;
...
DROP POLICY IF EXISTS "Anyone can view active zones" ON public.service_zones;
CREATE POLICY "Anyone can view active zones"
  ON public.service_zones FOR SELECT
  USING (is_active = true OR public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins manage zones - insert" ON public.service_zones;
CREATE POLICY "Admins manage zones - insert"
  ON public.service_zones FOR INSERT
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins manage zones - update" ON public.service_zones;
CREATE POLICY "Admins manage zones - update"
  ON public.service_zones FOR UPDATE
  USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins manage zones - delete" ON public.service_zones;
