...
DROP POLICY IF EXISTS "Admins manage quests" ON driver_quests;
CREATE POLICY "Admins manage quests" ON public.driver_quests FOR ALL
  USING (has_role(auth.uid(),'admin')) WITH CHECK (has_role(auth.uid(),'admin'));
DROP POLICY IF EXISTS "Drivers view active quests" ON driver_quests;
...
CREATE TABLE IF NOT EXISTS public.driver_quest_progress (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP POLICY IF EXISTS "Admins manage progress" ON driver_quest_progress;
CREATE POLICY "Admins manage progress" ON public.driver_quest_progress FOR ALL
  USING (has_role(auth.uid(),'admin')) WITH CHECK (has_role(auth.uid(),'admin'));
DROP POLICY IF EXISTS "Drivers view own progress" ON driver_quest_progress;
...
DROP POLICY IF EXISTS "Admins manage guarantees" ON driver_guarantees;
CREATE POLICY "Admins manage guarantees" ON public.driver_guarantees FOR ALL
  USING (has_role(auth.uid(),'admin')) WITH CHECK (has_role(auth.uid(),'admin'));
DROP POLICY IF EXISTS "Drivers view active guarantees" ON driver_guarantees;
...
DROP POLICY IF EXISTS "Admins manage streaks" ON streak_bonuses;
CREATE POLICY "Admins manage streaks" ON public.streak_bonuses FOR ALL
  USING (has_role(auth.uid(),'admin')) WITH CHECK (has_role(auth.uid(),'admin'));
DROP POLICY IF EXISTS "Drivers view active streaks" ON streak_bonuses;
...
DROP POLICY IF EXISTS "Admins manage pending payouts" ON pending_driver_payouts;
CREATE POLICY "Admins manage pending payouts" ON public.pending_driver_payouts
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Drivers see their pending payouts" ON pending_driver_payouts;
